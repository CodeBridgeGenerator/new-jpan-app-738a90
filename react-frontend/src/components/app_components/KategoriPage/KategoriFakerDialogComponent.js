...
import kategoriFakerFactory from "./kategoriFakerFactory";
...
const KategoriFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = kategoriFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("kategori").create(elem));
...
export default connect(mapState, mapDispatch)(KategoriFakerDialogComponent);

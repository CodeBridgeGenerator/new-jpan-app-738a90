...
const CalonCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,tajukLatihan: _entity?.tajukLatihan,status: _entity?.status,
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                ...
                <InputText id="tajukLatihan" className="w-full mb-3 p-inputtext-sm" value={_entity?.tajukLatihan} onChange={(e) => setValByKey("tajukLatihan", e.target.value)}  />
            </span>
            <small className="p-error">
